...
protected:
	...
	void _onPlayCallback() {
		this->state->isPlaying = this->mediaPlayer.isPlaying();
		std::cout << "onPlay callback" <<std::endl;
		if (this->getDuration() > 0) {
			this->state->isValid = this->mediaPlayer.isValid();
			this->state->isCompleted = false;
			this->state->position = this->getPosition();
	...
	void _onPauseCallback() {
		std::cout << "Puase callback" <<std::endl;
		this->state->isPlaying = this->mediaPlayer.isPlaying();
		if (this->getDuration() > 0) {
			this->state->position = this->getPosition();
			this->state->isValid = this->mediaPlayer.isValid();
	...
	void _onPositionCallback(float relativePosition) {
		...
		if (this->getDuration() > 0) {
			// DURATION TOTAL: 
			std::cout << this->getDuration() <<std::endl;
			// CURRENT POSITION:
		...
		// std::cout << relativePosition * this->mediaPlayer.length() <<std::endl;
	...
	void _onSeekableCallback(bool isSeekable) {
		if (this->getDuration() > 0) {
			std::cout << "Adelantando posición" <<std::endl;
